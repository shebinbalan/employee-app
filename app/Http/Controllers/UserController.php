...
class UserController extends Controller
{
   ...
   function action(Request $request)
   {
    if ($request->ajax()) {
      ...
      if ($total_row > 0) {
          $cards_per_row = 2; // Number of cards to display per row
          $card_counter = 0;
          ...
          // Check if there are any remaining cards that need to be closed
          if ($card_counter % $cards_per_row != 0) {
              $output .= '</div>';
          }
      } else {
  ...
  //      $users = Employee::where('name', 'LIKE', '%'.$searchTerm.'%')->get();
  //      return view('users.index', compact('users'));
  //  }
